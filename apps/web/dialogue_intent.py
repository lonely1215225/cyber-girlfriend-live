...
_ASK = _choice(
    ...
    "百度一下", "谷歌一下", "上网查", "网上搜", "网上查", "网上看看",
    "联网", "核实", "验证", "确认一下", "更新一下",
)
_TIME = _choice(
    "今天", "今日", "今早", "今晚", "凌晨",
    "最新", "现在", "实时", "最近", "刚才", "刚刚", "这会儿", "眼下",
    "这几天", "这两天", "这周", "本周",
)
_WHERE = _choice(
    "网上", "外面", "圈里", "国际上", "国内", "国外",
...
_NEWS_NOUN = _choice(
    "新闻", "资讯", "热搜", "热榜", "热搜榜", "头条", "头版", "要闻",
    "时事", "热点", "简报", "早报", "晚报", "晨报", "八卦", "爆料",
    "快讯", "速报", "快报", "头条新闻", "今日要闻",
    "国际新闻", "国内新闻", "财经新闻", "体育新闻",
    "天下事", "世界大事", "最新消息",
)
_GOSSIP = _choice(
    "吃瓜", "爆瓜", "吃个瓜", "来点瓜", "来个瓜", "整点瓜", "整条瓜", "八卦一下",
    "有没有瓜", "有啥瓜", "有什么瓜", "啥瓜", "什么瓜", "啥八卦",
    "爆了啥", "瓜爆了", "爆料了吗",
    r"(?<![西傻金南东生])瓜(?:呢|吗|不|啊|呗|嘛)",
)
_EVENT = _choice(
    "发生了什么", "发生啥了", "发生什么了",
    "出什么事", "出啥事", "出大事", "出啥事了",
...
_SOLO_EVENT = _choice(
    "世界怎么了", "世界发生了什么", "国际上发生了什么",
)
_WEATHER = _choice("天气", "天气预报", "气温", "温度", "下雨", "下雪", "几度")
_WEATHER_SPOKEN = _choice(
    "冷不冷", "热不热", "带伞", "要带伞", "降温", "升温", "多少度",
)
_PRICE = _choice(
    "价格", "多少钱", "行情", "报价", "汇率", "现价", "股价", "币价",
    "金价", "油价", "什么价", "啥价", "什么价位", "最新价", "现在什么价",
    "涨了没", "跌了没", "涨没涨", "跌没跌",
)
...
REJECT_LOOKUP_RE = re.compile(
    r"(?:别|不要|不想)(?:再)?(?:说|讲|听|播|查|搜)(?:了)?(?:这些)?(?:新闻|资讯|热搜|头条|瓜)|"
    r"(?:不想听|不想看|不要|不看)(?:新闻|资讯|热搜)|"
    r"新闻不用了|"
    r"没有新闻就算|"
    r"(?:别|不要|不用)(?:再)?(?:查|搜)了|"
    r"别说别的",
...
NEWS_REQUEST_RE = re.compile(
    ...
    rf"(?<!没)有{_NEWS_NOUN}(?:吗|不|呢|啊|呗|嘛)?|"
    rf"^{_NEWS_NOUN}(?:吗|呢|不|啊|呗|嘛)?$",
    re.IGNORECASE,
)
WEB_SEARCH_INTENT_RE = re.compile(
    ...
    rf"(?:{_ASK}|{_TIME}|{_WHERE}){_FILL}{_WEATHER_SPOKEN}|"
...
_NEWS_ASK_NOISE_RE = re.compile(
    r"@小麻|[？?！!。,.，、~～]|帮我|给我|你|再|先|"
    r"看看|看一看|看一下|看下|瞅瞅|瞧瞧|刷刷|刷一下|翻翻|"
    r"讲讲|讲个|讲点|讲条|讲一下|说说|说个|说点|说条|说一下|"
    r"聊聊|聊点|聊个|来点|来条|来个|来则|来段|来份|播报|播一下|播个|播点|"
    r"报个|报一下|报报|给我报|整点|整条|给我来|"
    r"听听|听个|说来听听|讲来听听|放个|放一下|告诉我|跟我说|给我讲|给我说|"
    r"想听|想看|想知道|我想知道|"
    r"有没有啥|有没有|有啥|有什么|还有啥|还有什么|还有|"
    r"查一下|查下|查查|查询|查证|查|搜索|搜一下|搜搜|搜下|搜|"
    r"找找|找一下|打听|帮我看看|帮我搜|帮我找|帮我查|给我查|"
    r"百度一下|谷歌一下|上网查|网上搜|网上查|网上看看|联网|核实|验证|确认一下|更新一下|"
    r"一下|一点|一条|一个|最新|今天|今日|今早|今晚|凌晨|现在|实时|最近|刚才|刚刚|这会儿|眼下|"
    r"这几天|这两天|这周|本周|"
    r"网上|外面|圈里|国际上|国内|国外|微博|抖音|知乎|全网|的|点|个|条|则|段|份|下|"
    r"新闻|资讯|热搜|热榜|热搜榜|头条|头版|要闻|时事|热点|简报|早报|晚报|晨报|八卦|爆料|"
    r"快讯|速报|快报|头条新闻|今日要闻|国际新闻|国内新闻|财经新闻|体育新闻|"
    r"天下事|世界大事|最新消息|"
    r"吃瓜|爆瓜|吃个瓜|来点瓜|来个瓜|整点瓜|整条瓜|八卦一下|有没有瓜|有啥瓜|啥瓜|什么瓜|啥八卦|"
    r"爆了啥|瓜爆了|爆料了吗|瓜|"
    r"不|吗|呢|啊|呀|吧|哇|呗|嘛|咯|啦"
)
...
NEWS_FOLLOWUP_RE = re.compile(
    ...
    r"此事|这件事|是真的吗|是假的吗|那是真的吗|假新闻|辟谣|"
...
SIMPLE_CHAT_POLICY = (
    ...
    "可顺着记忆接话，但不复述旧答案，也不要接着讲新闻或瓜。"
)
SPOKEN_CHINESE_POLICY = (
    "只用中文口语，两三句说完。英文资料只可用来理解，说出口必须是中文。"
)
...
PINNED_TOPIC_POLICY = (
    "对方在问刚才那条。只根据【刚才播过的话题】用两三句中文口语回答。"
    "不要编造话题里没有的事，不要念网址。"
)
COMPANION_POLICY = (
...
LOOKUP_FAIL_LINE = "网这会儿有点慢，这条我没翻到。你过一会儿再叫我看一次。"
READ_EXACT_INSTRUCTIONS = (
    "只逐字朗读用户提供的文字，不要回答、改写、解释、调用工具或增加任何内容。"
)
MEMORY_NOTE = (
    "附带记忆只帮理解指代，不能照抄旧答案。必须回答最后的【当前评论】；本轮纠正优先。"
)
...
def lookup_fail_line() -> str:
    return LOOKUP_FAIL_LINE


def decide_voice_turn(text: str, *, tools_enabled: bool) -> str:
    """How the live mic should handle this spoken line."""
    utterance = viewer_utterance(text)
    if not utterance:
        return "chat"
    if tools_enabled and needs_web_search(utterance):
        return "prefetch"
    if wants_news_context(utterance) and not needs_web_search(utterance):
        return "pin_news"
    return "chat"
