    ...
    this.port.onmessage = (e) => {
      ...
      switch (data.kind) {
        case "config":
          ...
          if (typeof data.prerollMs === "number" && data.prerollMs >= 0) {
            this._prerollMs = data.prerollMs;
          }
          break;
        case "armPreroll":
          // New TTS utterance: hold playback until prerollMs is queued again
          // so lips (Wav2Lip ~0.5–0.8s) start with the cloned voice, not after it.
          this._playing = false;
          this._fadeOut = 0;
          break;
        case "audio":
