...
const EMPTY_STATE_HTML = `<div id="chat-empty" class="chat-empty">${CHAT_BUBBLE_SVG}<span class="chat-empty-title">还没有对话</span><span class="chat-empty-hint">点击中间按钮开始说话</span></div>`;
...
export class ChatView {
  ...
  constructor(options = {}) {
    ...
    // Desktop keeps the transcript in a first-screen rail. The same DOM falls
    // back to the existing slide-over on narrower screens.
    this._dockMedia = window.matchMedia("(min-width: 901px)");
    this._syncDock = () => {
      const wasDocked = this._chatPanel.classList.contains("docked");
      const docked = this._dockMedia.matches;
      this._chatPanel.classList.toggle("docked", docked);
      if (docked) {
    ...
    this._dockMedia.addEventListener("change", this._syncDock);
    this._syncDock();
  }
  ...
  _appendHistImage(dataUrl) {
    ...
    const el = document.createElement("div");
    el.className = "hist-msg tool";
    el.innerHTML = `<div class="hist-role">画面快照</div><img class="hist-image" alt="发送给模型的摄像头快照" />`;
  ...
  onUserAudio(detail) {
    ...
    if (!container) {
      ...
      container.innerHTML = `
        <div class="hist-audio-label">发送的语音</div>
        <audio controls preload="metadata" aria-label="重播我的语音"></audio>
      `;
