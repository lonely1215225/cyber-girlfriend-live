...
export class S2sWsRealtimeClient extends EventTarget {
  ...
  async _setupAudio() {
    ...
    await ctx.audioWorklet.addModule(new URL("mic-capture.js?v=20260816o", base).href);
    await ctx.audioWorklet.addModule(new URL("audio-playback.js?v=20260816o", base).href);
    ...
    switch (type) {
      ...
      case "response.output_audio.delta": {
        ...
        if (!this._aiSpeaking) {
          ...
          this._markAudible();
          this._playbackNode?.port.postMessage({ kind: "armPreroll" });
        }
