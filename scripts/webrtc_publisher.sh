...
# Music must not use Opus lowdelay: that mode plus timestamp rewriting is
# what made background music and news onsets hitch on WebRTC while HTTP-FLV
# (which plays the original AAC) stayed smooth.
if [[ "$music" == "1" ]]; then
  opus_application="audio"
  opus_bitrate="${WEBRTC_MUSIC_OPUS_BITRATE:-64000}"
else
  opus_application="voip"
  opus_bitrate="${WEBRTC_OPUS_BITRATE:-48000}"
fi
...
while true; do
  ffmpeg -hide_banner -loglevel warning \
    ...
    -b:a "$opus_bitrate" -application "$opus_application" -frame_duration 20 \
